...
namespace AIHousingAssistant.Application.Services.ChatTools
{
    public class RagPipelineService : IRagPipelineService
    {
        ...
        private readonly IRagPandocPipelineProcessor _ragPipelineProcessor;
        private readonly IRagMDSharpPipelineProcessor _ragPipelineMDSharpProcessor;

        private readonly IRagDoclingPipelineProcessor _ragPipelineDoclingProcessor;
        ...
        public RagPipelineService(
            ...
            Kernel kernel,
            IRagPandocPipelineProcessor ragPipelineProcessor,
            IRagDoclingPipelineProcessor ragPipelineDoclingProcessor,
            IRagMDSharpPipelineProcessor ragPipelineMDSharpProcessor
            )
        {
            ...
            _ragPipelineDoclingProcessor = ragPipelineDoclingProcessor ?? throw new ArgumentNullException(nameof(ragPipelineDoclingProcessor));
            _ragPipelineMDSharpProcessor = ragPipelineMDSharpProcessor ?? throw new ArgumentNullException(nameof(ragPipelineMDSharpProcessor));
        ...
        public async Task ProcessDocumentByRagAsync(List<IFormFile> files, RagUiRequest ragUiRequest)
        {
            ...
            foreach (var file in files)
            {
                ...
                try
                {
                 ...
                 //   ProcessedDocument? docProcessed = null;

                    if (ragUiRequest.ToolsSearchBy == SearchToolType.RagPandDoc)
                    {
                        await _ragPipelineProcessor
                            .ExecutePipelineAsync(file, ragUiRequest);
                    }
                    else if (ragUiRequest.ToolsSearchBy == SearchToolType.RagMarkdownSharp)
                    {
                       await _ragPipelineMDSharpProcessor
                            .ExecutePipelineAsync(file, ragUiRequest);
                    }
                    else if (ragUiRequest.ToolsSearchBy == SearchToolType.RagDocling)
                    {
                        await _ragPipelineDoclingProcessor
                            .ExecutePipelineAsync(file, ragUiRequest);
                    }
...
2. Use Markdown table for lists/data..
