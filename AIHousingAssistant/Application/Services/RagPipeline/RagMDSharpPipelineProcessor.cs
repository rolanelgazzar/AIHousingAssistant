...
using AIHousingAssistant.Application.Services.RagPipeline.Handlers.MarkDownSharp;

namespace AIHousingAssistant.Application.Services.RagPipeline
{
    public class RagMDSharpPipelineProcessor : IRagMDSharpPipelineProcessor
    {
        ...
        public RagMDSharpPipelineProcessor(
        ...
        public async Task<RagPipelineRequest> ExecutePipelineAsync(IFormFile file, RagUiRequest uiRequest)
        {
            ...
            var extraction = new MarkDownSharpHandler();
            var metadata = new FileMetaDataHandler();
            var normalization = new TextNormalizationMDSharpHandler();
